...
def count_by_status(tasks):
    return Counter(task["status"] for task in tasks).most_common(1)

def blocked_tasks(tasks):
    return [task["id"] for task in tasks if task["status"] == "blocked"]

def total_hours_by_assignee(tasks):
    assignee_hours = defaultdict(list)
    for task in tasks:
        assignee_hours[task["assignee"]].append(task["hours"])
    result = {}
...
def busiest_person(tasks):
    return Counter(task["assignee"] for task in tasks if task["priority"] == "high").most_common(1)
...
def completion_rate_by_assignee(tasks):
    ...
    return {a: round(done[a]/total[a] * 100, 1) for a in done}
...
def generate_report(tasks):
    ...
    with open("report2.json", "w") as file:
